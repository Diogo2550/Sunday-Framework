...
class QueryBuilder {
    ...
    public function insertWhereFieldsOnQuery(array $whereArray, string $tableName) {
        $this->query[] = "WHERE";
        foreach(array_keys($whereArray) as $i => $key) {
            $query = "";
            ...
            if(!isset($whereArray[$key])) {
                throw new Exception("Propriedade $key com valor vazio detectado.");
            }
            $query .= "$tableName.$key='$whereArray[$key]'";
